/**
 * Maps Ashdod license data to the TreePermit model format
 * Follows strict format without adding additional fields
 */
function mapToTreePermitModel(ashdodLicense) {
    // Extract core data
    const {
        address,
        licenseType,
    ...
    let data = {};
    if (ashdodLicense.resourceData) {
        if (typeof ashdodLicense.resourceData === 'string') {
            try {
                data = JSON.parse(ashdodLicense.resourceData);
            } catch (error) {
                console.warn(`Error parsing resourceData: ${error.message}`);
            }
        } else {
            data = ashdodLicense.resourceData;
        }
    }
    ...
    const resourceId = 'ASHDOD' + data.permit_number || permitNumber || generateResourceId(filename, date, address);

    // Return the strictly formatted model
    return {
        permitNumber: data.permit_number || permitNumber || resourceId,
        licenseType: mappedLicenseType,
        address: data.street || address,
        houseNumber: data.house_number || extractHouseNumber(address),
        settlement: data.settlement || 'אשדוד',
        gush: data.gush || '',
        helka: data.helka || '',
        reasonShort: data.reason_short || '',
        reasonDetailed: data.reason_detailed || '',
        licenseOwnerName: data.license_owner_name || '',
...
function generateResourceId(filename, date, address) {
    ...
    return `ASHDOD-${dateString}-${addressHash}-${baseFilename.substring(0, 8)}`;
}
